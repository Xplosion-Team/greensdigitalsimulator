...
try:
    ...
    df_simulation    # Save results
    df_simulation.to_csv("simulation_results_final.csv", index=False)
    print("Simulation completed successfully!")
    
    # Export results to CSV as backup
    csv_filename = "simulation_results.csv"
    df_simulation.to_csv(csv_filename, index=False)
...
try:
    ...
    output_filename = "img/example_digitaltwin" + str(myDigitalTwin.n_digitalTwin) + ".png"
    if not os.path.exists("img"):
        os.makedirs("img")
        
    plt.savefig(
        output_filename,
        dpi=300,  # Change DPI for different quality (300, 600, etc.)
    )
