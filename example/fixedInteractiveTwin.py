...
df_original = pd.read_csv("example_model/data_example.csv")
df_original = df_original[~df_original.is_train]
...
fig.suptitle('Interactive T1D Digital Twin - Real-time Control', fontsize=16, fontweight='bold')
...
def update_digital_twin(val):
    ...
    # Use the original data to avoid indexing issues
    df_simulation = myDigitalTwin.simulate(df_original.iloc[1 * 12 * 24 : 2 * 12 * 24])
...
def init():
    ...
    global df_simulation
    # Initialize with current digital twin
    myDigitalTwin = DigitalTwin(n_digitalTwin=current_digital_twin)
    df_simulation = myDigitalTwin.simulate(df_original.iloc[1 * 12 * 24 : 2 * 12 * 24])
...
def animate(frame):
    ...
    global animation_running, df_simulation
    ...
    glucose_text.set_color(color)  # Set color separately
...
# Create animation
print("Creating fixed interactive animation... This may take a moment...")
ani = FuncAnimation(fig, animate, init_func=init, frames=len(df_original.iloc[1 * 12 * 24 : 2 * 12 * 24]), 
...
# Show the animation
print("Fixed interactive animation ready!")
print("Controls:")
